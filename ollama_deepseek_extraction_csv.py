...
# pattern regex pour pre-processing données des fichiers csv
pattern = re.compile(
    r'http\S+|www\.\S+|'      # supprimer urls
    r'#\w+|'                  # supprimer hashtags
    r'['
        ...
        u"\U000024C2-\U0001F251"  # caractères encerclés
        r']+', 
        flags=re.UNICODE
)
...
def load_prompt(prompt): # charger le prompt que le llm va lire
    with open(prompt, 'r') as f:
        return f.read()

def read_files(folder): # charger les fichiers csv
...
def pre_processing(file_path):
    with open(file_path, mode='r', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            merged_text = f"{titre} {description} {message}" # merger les colonnes titre, description et message
...
def process_csv_line(line, prompt_template): # lire les fichiers csv ligne par ligne

    real_prompt = prompt_template.format(input_text=line)  # la ligne du fichier csv devient le input
